...
class Plateaus_Spectra(object):
    ...
    def set_frame(self):
        ...
        for j in range(self.Nd):
            for i in range(self.N_s1):
                ...
                self.F['ax' + idx].tick_params(
                  'both', length=8, width=1, which='major', pad=8)
                self.F['ax' + idx].tick_params(
                  'both', length=4, width=1, which='minor', pad=8)
    ...
    def save_figure(self):        
        if self.save_fig:
            if self.region == 'optical':
                fname = 'Fig_11fe_C-plateaus-spectra.pdf'
            elif self.region == 'NIR':
                fname = 'Fig_11fe_C-plateaus-spectra_NIR.pdf'
