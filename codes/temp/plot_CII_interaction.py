...
class Analyse_Iter(object):
    ...
    def analyse_data(self):
        ...
        for shell in self.shell:
            ...
            #Compute number fraction of C_II that absorb light in the carbon
            #trough region and re-emit it outside that range.
            frac = LLI(self.mode, shell, 6000., 6600., 6, 1,
                       last_line_interaction_in_id,
                       last_line_interaction_out_id,
                       last_line_interaction_shell_id,
                       output_nu, lines).update_last_interaction_filter()
            
            
            '''
            frac = LLI(self.mode, shell, 6, 1,
                       last_line_interaction_in_id,
                       last_line_interaction_out_id,
                       last_line_interaction_shell_id,
                       output_nu, lines)
    ...
    def plot_data(self):
        ...
        self.ax1.plot(self.model_v_inner, self.trough_frac, ls='None', marker='s',
...
Analyse_Iter(y2_mode='T', mode='line_in_nu', lm='macroatom', L='8.505',
             Fes='0.04', show_fig=True, save_fig=False)
