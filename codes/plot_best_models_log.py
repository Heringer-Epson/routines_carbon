...
class Plot_Best(object):
    ...
    def __init__(self, show_fig=True, save_fig=False):
        ...
        self.F['axi_o0'] = self.fig.add_axes([xloc, 0.763, dx, dy])
        self.F['axi_o1'] = self.fig.add_axes([xloc, 0.605, dx, dy])
        self.F['axi_o2'] = self.fig.add_axes([xloc, 0.442, dx, dy])
        self.F['axi_o3'] = self.fig.add_axes([xloc, 0.283, dx, dy])
        self.F['axi_o4'] = self.fig.add_axes([xloc, 0.122, dx, dy])
    ...
    def set_fig_frame(self):
        ...
        y_label = r'$\mathrm{Relative \ log \ f}_{\lambda}$'
        ...
        for j in range(Nd):
            ...
            self.F['ax' + idx].set_ylim(0.01, 10.)      
    ...
    def save_figure(self):        
        if self.save_fig:
            ...
            plt.savefig(directory + 'Fig_11fe_best_log.pdf',
...
class Add_Curves(object):
    ...
    def set_fig_frame(self):
        ...
        self.axi_o.tick_params(axis='y', which='major', labelsize=fs - 8., pad=8)      
        self.axi_o.tick_params(axis='x', which='major', labelsize=fs - 8., pad=8)
    ...
    def add_texp_text(self):            
        self.ax.text(9500., 3.5, r'$t_{\rm{exp}}=' + self.t_exp + '\\ \\rm{d}$',
                     fontsize=20., horizontalalignment='left', color='k')
        self.ax.text(1750., 1.5, r'$\mathbf{' + self.panel[self.idx] + '}$',
                     fontsize=20., horizontalalignment='left', color='k')
...
if __name__ == '__main__':
    Plot_Best(show_fig=False, save_fig=True)
